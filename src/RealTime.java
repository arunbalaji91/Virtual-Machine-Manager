//RealTime.java                                                                                             asivak01@syr.edu
//------------------------------------------------------------------------------------------------------------------
...
public class RealTime extends ConnectedVimServiceBase {
    ...
    void displayValues(List<PerfEntityMetricBase> values,
                       Map<Integer, PerfCounterInfo> counters) throws IOException {
    ...
    void doRealTime() throws IOException, RuntimeFaultFaultMsg, InterruptedException {
        ...
        if (vmmor != null) {
            ...
            Map<Integer, PerfCounterInfo> mcounters =
                    new HashMap<Integer, PerfCounterInfo>();
            int i = 14;
            while (true) {
                if (i > vmCpuCounters.size()) {
                    ...
                    if (memcounter.size() > 0)
                    {
                    	
                    	PerfCounterInfo memInfo = memcounter.get(1);
                        mcounters.put(new Integer(memInfo.getKey()), memInfo);
                    }
                    break;
                }
            }
            ...
            if (listpermeid != null) {
                for (int index = 0; index < listpermeid.size(); ++index) {
                    ...
                    if (mcounters.containsKey(new Integer(listpermeid.get(index)
                            .getCounterId()))) {
    ...
    void monitorPerformance(ManagedObjectReference pmRef,
        ...
        PerfQuerySpec q_Spec = new PerfQuerySpec();
        ...
        while (true) {
            ...
            if (p_Values != null) {
                displayValues(p_Values, mcounters);
            } 
    ...
    @Action
    public void run(com.vmware.vim25.ServiceContent serviceContent, com.vmware.vim25.VimPortType vimPort) throws RuntimeFaultFaultMsg, IOException, InterruptedException {
    	text_file = new File("VM_usage.txt"); 
        ...
        this.setVirtualmachinename("ArunVM1");
        doRealTime();
    }
}
